...
<body>
	...
	<div style="margin-left: 25%; padding: 1px 16px; height: 1000px;">
		...
		<form action="searchTeacher" style="margin-bottom: 10px; text-align: center" >
			<input type="number" name="teId"> <input type="submit"
				value="search by id">
		</form>
		
		<form action="searchTeachersByName" style="margin-bottom: 10px; text-align: center">
		...
		<table>
			...
			<c:forEach items="${list}" var="t">
				<tr>
					...
					<td>${t.teName}</td>
					<td>${t.teDOB}</td>
					<td>${t.teGender?"Male":"Female"}</td>
					<td>${t.tePhone}</td>
					<td>${t.teMail}</td>
					<td>${t.salary}</td>
					<td><a href="detailTeacher?teId=${t.teId}">detail</a></td>
					<td><a href="deleteTeacher?teId=${t.teId}"
						onclick="return confirm('Are you sure?')">delete</a></td>
				</tr>
			</c:forEach>
		</table>

		<a class="button" href="initInsertT" style="margin: 15px">Add New
			Teacher</a>
	</div>
</body>
</html>